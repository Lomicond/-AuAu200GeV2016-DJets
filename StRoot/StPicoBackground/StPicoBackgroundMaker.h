#ifndef StPicoBackgroundMaker_h
#define StPicoBackgroundMaker_h
...
class StPicoBackgroundMaker : public StMaker
{
  private:
...
Int_t fNumberOfEventsToOverLay = 1; ///THIS
    ...
    ClassDef(StPicoBackgroundMaker, 1)

};
/*
inline int StPicoBackgroundMaker::getEntries() const {
  return mChain? mChain->GetEntries() : 0;
}
*/
inline void StPicoBackgroundMaker::setCutETmin(float min){
    fETmincut = min;
}

inline void StPicoBackgroundMaker::setMaxDcaZHadronCorr(float max) {
    maxdcazhadroncorr = max;
}

inline void StPicoBackgroundMaker::setGhostMaxrap(float fGhostMaxrap) {
    StPicoBackgroundMaker::fGhostMaxrap = fGhostMaxrap;
}

inline void StPicoBackgroundMaker::setNJetsRemove(int nJetsRemove) {
    StPicoBackgroundMaker::nJetsRemove = nJetsRemove;
}
/*
inline void StPicoBackgroundMaker::readNextEvent(){
    mChain->GetEntry(mEventCounter++);
}
*/
inline void StPicoBackgroundMaker::setMaxNeutralFraction(float max) {
    maxneutralfrac = max;
}

inline void StPicoBackgroundMaker::setHadronCorr(float corr) {
    fHadronCorr = corr;
}

inline void StPicoBackgroundMaker::setHFCuts(StHFCuts* cuts) {
    mHFCuts = cuts;
}

inline void StPicoBackgroundMaker::setOnlyTrackBasedJets(bool OTBJets) {
