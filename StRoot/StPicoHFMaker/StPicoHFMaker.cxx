...
void StPicoHFMaker::createTertiaryK0Shorts() {
  ...
  for (unsigned short idxPion1 = 0; idxPion1 < mIdxPicoPions.size(); ++idxPion1) {
    ...
    if (!mHFCuts->cutMinDcaToPrimVertexTertiary(pion1, StHFCuts::kPion,mPicoDst))
      continue;
	...
	continue;
      ...
      if (!mHFCuts->cutMinDcaToPrimVertexTertiary(pion2, StHFCuts::kPion,mPicoDst))
	continue;
...
void StPicoHFMaker::createTertiaryLambdas() {
  ...
  for (unsigned short idxProton = 0; idxProton < mIdxPicoProtons.size(); ++idxProton) {
    ...
    if (!mHFCuts->cutMinDcaToPrimVertexTertiary(proton, StHFCuts::kProton,mPicoDst))
      continue;
	...
	continue;
      ...
      if (!mHFCuts->cutMinDcaToPrimVertexTertiary(pion, StHFCuts::kPion,mPicoDst))
	continue;
...
bool StPicoHFMaker::setupEvent() {
  ...
  mPrimVtx = StThreeVectorF(mPicoEvent->primaryVertex().x(),mPicoEvent->primaryVertex().y(),mPicoEvent->primaryVertex().z());
