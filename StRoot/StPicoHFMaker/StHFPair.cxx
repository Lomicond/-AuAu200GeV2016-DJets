...
StHFPair::StHFPair(StPicoTrack const * const particle1, StPicoTrack const * const particle2,
  ...
  StThreeVectorF const p2Mom = StThreeVectorF(
                                                (p2Helix.momentum(bField * kilogauss)).x(),
  ...
  StThreeVectorF const p1AtDcaToP2 = (useStraightLine) ? StThreeVectorF(p1StraightLine.at(ss.first)) : StThreeVectorF((p1Helix.at(ss.first)).x(), (p1Helix.at(ss.first)).y(), (p1Helix.at(ss.first)).z());

  //StThreeVectorF const p2AtDcaToP1 = (useStraightLine) ?                p2StraightLine.at(ss.second) :                  p2Helix.at(ss.second);
    StThreeVectorF const p2AtDcaToP1 = (useStraightLine) ? StThreeVectorF(p2StraightLine.at(ss.second)) : StThreeVectorF((p2Helix.at(ss.second)).x(), (p2Helix.at(ss.second)).y(), (p2Helix.at(ss.second)).z());
  ...
  StThreeVectorF const p1MomAtDca = StThreeVectorF(  (p1Helix.momentumAt(ss.first,  bField * kilogauss)).x(),
                                                (p1Helix.momentumAt(ss.first,  bField * kilogauss)).y(),
                                                (p1Helix.momentumAt(ss.first,  bField * kilogauss)).z()
                                              );
  StThreeVectorF const p2MomAtDca = StThreeVectorF(
                                                (p2Helix.momentumAt(ss.second, bField * kilogauss)).x(),
                                                (p2Helix.momentumAt(ss.second, bField * kilogauss)).y(),
                                                (p2Helix.momentumAt(ss.second, bField * kilogauss)).z()
                                              );
  ...
  //    if decay vertex is a tertiary vertex
  //    -> only rough estimate -> needs to be updated after secondary vertex is found
  //  mParticle1Dca = (StThreeVectorF(p1Helix.origin() - vtx).mag();
  //  mParticle2Dca = (StThreeVectorF(p2Helix.origin() - vtx).mag();
...
StHFPair::StHFPair(StPicoTrack const * const particle1, StHFPair const * const particle2,
  ...
  StPicoPhysicalHelix p2Helix(TVector3(p2Mom.x(),p2Mom.y(),p2Mom.z()),TVector3(p2Origin.x(),p2Origin.y(),p2Origin.z()),bField*kilogauss, p2Charge);
 ...
 // p2Helix.moveOrigin(p2Helix.pathLength(vtx));
  ...
  StPhysicalHelixD const p1StraightLine(p1Mom, StThreeVectorF(p1Helix.origin().x(),p1Helix.origin().y(),p1Helix.origin().z()), 0, particle1->charge());
  StPhysicalHelixD const p2StraightLine(p2Mom, StThreeVectorF(p2Helix.origin().x(),p2Helix.origin().y(),p2Helix.origin().z()), 0, p2Charge);
  ...
  StThreeVectorF const p2MomAtDca = StThreeVectorF( (p2Helix.momentumAt(ss.second,  bField * kilogauss).x()),
                                                    (p2Helix.momentumAt(ss.second,  bField * kilogauss).y()),
                                                    (p2Helix.momentumAt(ss.second,  bField * kilogauss).z())
                                                                                                            );
  ...
  // -- calculate pointing angle and decay length with respect to primary vertex
  StThreeVectorF const vtxToV0 = mDecayVertex - vtx;
  //StThreeVectorF const vtxToV0 = mDecayVertex - StThreeVectorF(vtx.x(),vtx.y(),vtx.z());
  mPointingAngle = vtxToV0.angle(mLorentzVector.vect());
  mDecayLength = vtxToV0.mag();
...
float StHFPair::particle1Dca(StPicoTrack const * p1track, StThreeVectorF const & vtx2,float const bField) const{
  ...
  p1Helix.moveOrigin(p1Helix.pathLength(vtx2.x(),vtx2.y()));
  //float const nParticle1Dca = (p1Helix.origin() - vtx2).mag();
  float const nParticle1Dca = (p1Helix.origin() - TVector3(vtx2.x(),vtx2.y(),vtx2.z())).Mag();
...
float StHFPair::particle2Dca(StPicoTrack const * p2track, StThreeVectorF const & vtx2,float const bField) const{
  ...
  p2Helix.moveOrigin(p2Helix.pathLength(vtx2.x(),vtx2.y()));
  //float const nParticle2Dca = (p2Helix.origin() - vtx2).mag();
  float const nParticle2Dca = (p2Helix.origin() - TVector3(vtx2.x(),vtx2.y(),vtx2.z())).Mag();
