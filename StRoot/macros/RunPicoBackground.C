...
#include "StPicoBackground/StPicoBackgroundMaker.h"
...
void RunPicoBackground(string d0list="testD0.list", string pico="testPico.list",
 string outFileName="Test_AnaMaker.root"/*, string badRunListFileName = "picoList_bad_MB.list"*/,int pYear = 2014, bool Testing = true)
{
   ...
   outFileName=Form("Test_BackgroundMaker_%.d.root",pYear);
   }
   ...
   gROOT->LoadMacro("StRoot/macros/loadSharedAnalysisLibraries_B.C");
   ...
   StPicoBackgroundMaker*  picoBackgroundMaker = new StPicoBackgroundMaker("picoBackgroundMaker",/* d0list.c_str(),*/ outFileName.c_str(), picoDstMaker,grefmultCorrUtil,pYear, false, "");
   ...
   // -------------- USER variables -------------------------
   picoBackgroundMaker->setHFCuts(d0Cuts);
   picoBackgroundMaker->setCutETmin(0.2);
   picoBackgroundMaker->setHadronCorr(1.);
   picoBackgroundMaker->setOnlyTrackBasedJets(false);
   picoBackgroundMaker->setMaxDcaZHadronCorr(3.0); //cm, max DCA_z for global tracks used for hadronic correction 
   picoBackgroundMaker->setNJetsRemove(1);
   picoBackgroundMaker->setGhostMaxrap(1.0);
   picoBackgroundMaker->setMaxNeutralFraction(0.95);
  ...
  // {
  /// if (nEntries>picoDstMaker->chain()->GetEntries()) ;
   
 //  }
///nEntries = picoDstMaker->chain()->GetEntries()/10;
   nEntries = picoDstMaker->chain()->GetEntries();
