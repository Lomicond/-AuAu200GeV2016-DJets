...
folder_path = 'log'  # Upravte cestu k vaší složce
...
for file_name in files:
    if file_name.endswith('.log') and '1_Tower_' in file_name:
...
for i in range(1, max_number + 1):
    if i not in [int(file.split('_')[2].split('.')[0]) for file in files if file.endswith('.log') and '1_Tower_' in file]:
